...
TEMPLATE_ROOT = 'sample_generator_templates/'
NUMBER_OF_SUBJECTS = 50*10
NUMBER_OF_ENCOUNTERS = 100*10
...
concepts = []
with open(TEMPLATE_ROOT + 'concepts.txt') as file:
	concepts = file.read().splitlines()

concept_values = []
with open(TEMPLATE_ROOT + 'concept_values.txt') as file:
	concept_values = file.read().splitlines()
...
for i in range(0, NUMBER_OF_ENCOUNTERS):
	...
	for i in range(0, random.randint(0,4)):
		#Create a Random Observation with random concept
		observation_uuid = uuid.uuid4()
		random_concept = random.randint(0,len(concepts)-1)
		observation_value_list = concept_values[random_concept].split(";")
		observation_value = observation_value_list[random.randint(0,len(observation_value_list)-1)]
		if (observation_value == 'Date'):
			observation_value = datetime.datetime.now() - datetime.timedelta(days=random.randint(0,OLDEST_PERSON*365), hours=random.randint(0,23), minutes=random.randint(0,59))
			while (observation_value > created_date):
				observation_value = datetime.datetime.now() - datetime.timedelta(days=random.randint(0,OLDEST_PERSON*365), hours=random.randint(0,23), minutes=random.randint(0,59))
		observation_value = str(observation_value)
		observation_concept = concepts[random_concept]
		observation_node = "2a" + str(i)
		observation_dictionary = {'observation_number':observation_counter, 'observation_node':observation_node, 'observation_value':observation_value, 'observation_concept':observation_concept, 'observation_uuid':observation_uuid, 'created_date':created_date, 'encounter_uuid': encounter_uuid}
		observation_json = observation_json + observation_template.substitute(observation_dictionary) + ",\n"
		observation_counter = observation_counter + 1;		
